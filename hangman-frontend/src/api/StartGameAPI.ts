...
export class StartGameAPI {
  ...
  public static async send(gameToken: string, playerToken: string, hostUuid: string, numberOfIncorrectGuesses: number): Promise<GameData> {
    if (this.sending) {
    ...
    return await new BaseApi().put<{ status }>("{0}:{1}/api/game/{2}/start"
      ...
      .replace("{2}", gameToken), {"host": playerToken, "uuid": hostUuid, "numberOfIncorrectGuesses": numberOfIncorrectGuesses})
      .then(response => {
        this.sending = false;
        return response;
      });
